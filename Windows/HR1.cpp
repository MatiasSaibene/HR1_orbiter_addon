...
void HR1::UpdateDockingPortAnimation(double simdt) {
    if (docking_port_status >= DCK_CLOSING) {
        double da = simdt * LANDING_GEAR_OPERATING_SPEED;
        if (docking_port_status == DCK_CLOSING) {
            if (docking_port_proc > 0.0) docking_port_proc = max(0.0, docking_port_proc - da);
            else docking_port_status = DCK_CLOSED;
        } else {
            if (docking_port_proc < 1.0) docking_port_proc = min(1.0, docking_port_proc + da);
...
int HR1::clbkConsumeBufferedKey(DWORD key, bool down, char *kstate){
...
void HR1::ScalePanel (PANELHANDLE hPanel, DWORD viewW, DWORD viewH)
{
